...
p1 <- autoplot(ts(diff(dni_06_2019_1$dni, 29))) + 
  th + labs(x='', y='DNI', title='DNI Differenced Time Series for June 1st-7th, 2019')
p2 <- autoplot(ts(diff(dni_06_2019_2$dni, 29))) + 
  th + labs(x='', y='DNI', title='DNI Differenced Time Series for June 3rd-7th, 2019')
p3 <- autoplot(ts(diff(dni_06_2019_3$dni, 29))) + 
  th + labs(x='', y='DNI', title='DNI Differenced Time Series for June 5th-7th, 2019')

p <- arrangeGrob(p1,p2,p3)
ggsave('../plots/ts_diff.png', p, bg='transparent')
...
g2 <- ggplot(P_df_2, aes(x=x, y=y)) + 
  ...
  annotate('text', x=P_df_2$x[ix_2]+0.07, y=P_df_2$y[ix_2]-300000, 
           label=TeX(sprintf('$\\omega = %.4f$', P_df_1$x[ix_2])), size=3) + 
  geom_line(size=0.25) + 
  scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
